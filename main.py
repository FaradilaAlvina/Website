...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'qardiosis-key.json'
                    ...
                    if account:
                        ...
                        try :
                            with con.cursor() as cur:
                                ...
                                print(pw)
                                query = f"INSERT INTO user (first_name, last_name, email, username, password, medical_number, img_profile) VALUES ('{first_name}', '{last_name}', '{email}', '{username}', '{pw}', '{medical_number}', 'dafault_photo_profile.png' )"
                                print(query)
                                print("check 5")
                                account = cur.execute(query)
                                print(account)
                                con.commit()
                                print("check commit")
                                print("check 6")
            ...
            if request.method == 'POST':
                try:
                    ...
                    with con.cursor() as cur:
                        ...
                        con.commit()
                        con.close()
                        print("Success to add data patient to database")
                except Exception as e:
                    print (e)
            return ('', 204)
    except Exception as e:
...
## Download History ##
@app.route('/history/download/<int:id_patient>', methods=['GET'])
def download_data(id_patient):
    ...
    with con.cursor() as cur:
        ...
        print(url_img)
        print("itu url img kamu")
        download_from_storage(url_img, url_img, bucket_name)
        con.close()
    return ('', 204)
